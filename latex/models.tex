...
\int \limits_{V} \alpha_{i} \frac{\partial C_{i}}{\partial t} dV - \oint \limits_{S} \beta_{i} \vec{\nabla}C_{i} \; \vec{dS} = 0,
\end{eqnarray}
where
\begin{eqnarray}
\begin{gathered}
\label{eq:alpha}
\alpha_{i}  =  1+\frac{1 - \phi}{\phi}C_{si}', \: \beta_{i}  = D_{i}.
\end{gathered}
\end{eqnarray}
...
and by applying the divergence theorem, the following equation can be obtained
  \begin{eqnarray}
 \label{eq:diffusuin_multicomp_integral}
 \int \limits_{V} \alpha_{i} \frac{\partial C_{i}}{\partial t} d V - \oint \limits_{S} \beta_{i} \vec{\nabla}C_{i} \; \vec{dS} = 0,
 \end{eqnarray}
where  
 \begin{eqnarray}
 \begin{gathered}
